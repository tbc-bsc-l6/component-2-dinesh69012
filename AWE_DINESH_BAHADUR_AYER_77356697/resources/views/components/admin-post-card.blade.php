...
{{--        <p class="date">{{ $post->updated_at->format('d.m.Y') }} by {{ $post->user->firstname . ' ' . $post->user->lastname }}</p>--}}
    ...
    <div class="actions">
        <a href="{{ route('post.show', $post->slug) }}" class="read_more">Go <i class="fa-solid fa-angles-right"></i></a>
        @can('post-edit')
        ...
        @can('post-highlight')
            @if($post->is_highlighted)
                ...
                <button onClick="document.getElementById('highlight_{{ $post->id }}').submit();" class="highlight deselect">Uncheck <i class="fa-solid fa-star"></i></button>
            @else
